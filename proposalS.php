					  ...
					  <div class="ace-settings-container" id="ace-settings-container">
						 ...
						 if(isset($_REQUEST['search']))
						 {
								  ...
								  if($workshop1=='' && $workshop2=='')
									...
									$querySelect=mysqli_query($dba,"select *  from report left join student ON report.matricNum = student.matricNum left join proposal ON report.reportID = proposal.reportID 
									...
									proposal.statusProject ='Approve'") or die();	
									...
									$selectCount = "select count(*) AS Total from proposal left join report ON proposal.reportID = report.reportID left join student ON report.matricNum = student.matricNum where (student.typeOfWorkshop='1' OR student.typeOfWorkshop ='2' ) AND (student.course = '".$course1."' OR student.course = '".$course2."' OR student.course = '".$course3."' OR student.course = '".$course4."' OR student.course = '".$course5."' OR student.course = '".$course6."' OR student.course = '".$course7."') AND proposal.statusProject ='Approve'";
						 ...
						 else if($course1=='' && $course2=='' && $course3=='' && $course4=='' && $course5=='' && $course6=='' && $course7=='')
							{
									...
									$selectCount = "select count(*) AS Total from proposal left join report ON proposal.reportID = report.reportID left join student ON report.matricNum = student.matricNum where (course = 'BITS' OR course = 'BITM' OR course = 'BITE' OR course = 'BITD' OR course = 'BITC' OR course = 'BITZ' OR course = 'BITI') AND
								    proposal.statusProject ='Approve'";
									$essms_selectCount =mysqli_query($dba,$selectCount) or die (mysqli_error());
									$rowCount = mysqli_fetch_array($essms_selectCount);								
							...
							else
							{
									...
									$selectCount = "select count(*) AS Total from proposal left join report ON proposal.reportID = report.reportID left join student ON report.matricNum = student.matricNum where (student.typeOfWorkshop='".$workshop1."' OR student.typeOfWorkshop ='".$workshop2."' ) AND
									(student.course = '".$course1."' OR student.course = '".$course2."' OR student.course = '".$course3."' OR student.course = '".$course4."' OR student.course = '".$course5."' OR student.course = '".$course6."' OR student.course = '".$course7."') AND
									proposal.statusProject ='Approve'";
									$essms_selectCount =mysqli_query($dba,$selectCount) or die (mysqli_error());
									$rowCount = mysqli_fetch_array($essms_selectCount);	
						...
						else
						{
									...
									$selectCount = "select count(*) AS Total from proposal left join report ON proposal.reportID = report.reportID left join student ON report.matricNum = student.matricNum where (student.typeOfWorkshop='1' OR student.typeOfWorkshop ='2' ) AND
									(course = 'BITS' OR course = 'BITM' OR course = 'BITE' OR course = 'BITD' OR course = 'BITC' OR course = 'BITZ' OR course = 'BITI') AND
									proposal.statusProject ='Approve'";
									$essms_selectCount =mysqli_query($dba,$selectCount) or die (mysqli_error());
									$rowCount = mysqli_fetch_array($essms_selectCount);									
		...
		<div class="page-header">
							<h1>
								Student's Proposal
									...
									<form class="form-horizontal" role="form" method="post" action="proposalS.php">
							...
							<div class="col-xs-12">
								<!-- PAGE CONTENT BEGINS -->
		                   <p><a href="javascript:window.open('assignDueSubmisionP.php','mywindowtitle','width=800,height=700')"><input type="button" value="Assign Submission Date"></a></p>
										...
										<table id="dynamic-table" class="table table-striped table-bordered table-hover">
												...
												<tbody>
													<tr>
														...
														<td><p>	<i class="menu-icon fa fa-upload"></i>&nbsp;<a href="Documents/Proposal/<?php echo $row['sourceP']; ?>"  ><?php echo $row['sourceP']; ?></a></p></td>
